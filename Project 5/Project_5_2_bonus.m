...
%AIM    : To simulate a protocol similar to the one used in Ethernet, with
%retransmission of packets with varying probability.
%NAME   : Tanmayan Pande
...
%18. retrans: A variable which stores the number of times a packet has been
...
for i = 1:length(lambda)
    ...
    for k = 1:10000
        ...
        %packets which are not transmitted are sent back to the buffer
        ...
        for j = 1:N
            ...
            if(buff(j, B) ~= 0)
                ...
                if(a <= p^retrans(j))
                    ...
                    transqueue(point, 3) = retrans(j, 1);
                    point = point + 1;
                end
            end
        end
        ...
        if(transqueue(2, 1) ~= 0)
            Col(i) = Col(i) + 1;
            for o = 2:point - 1
                %For all the packets in trasnqueue, update the values of
                %retrans and push the current packet to the end of the
                %buffer of the packet radio it came from.
                retrans(transqueue(o, 1), 1) = retrans(transqueue(o, 1), 1) + 1;
                retrans(transqueue(o, 1), :) = [retrans(transqueue(o, 1), 2:B) retrans(transqueue(o, 1), 1)];
                buff(transqueue(o, 1), :) = [buff(transqueue(o, 1), 2:B) transqueue(o, 2)];
            end
                ...
                %buffer stats
                last(transqueue(1,1)) = last(transqueue(1,1)) - 1;
                buff(transqueue(1,1), :) = [buff(transqueue(1,1), 2:end) 0];
                retrans(transqueue(1,1), 1) = 1;
            end
        end
